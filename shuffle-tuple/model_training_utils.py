...
class WatchTrainer():
    ...
    def __init__(self, solverPath, solver=None, checkSolver=True, verbose=True):
        assert (os.path.isfile(solverPath)) , 'solver: {} does not exist'.format(solverPath);
    ...
    def parse_solver(self):
        ...
        assert( os.path.isdir(snapPath) ), '{} does not exist'.format(snapPath);
    ...
    def model_weight_diffs(self):
        net = self.solver.net;
        layers = list(net.params.keys()); #params is an ordered dict, so keys are ordered
        currwtdiffs = [];
        for layer in layers:
    ...
    def train_model(self, max_iters, log_iter, snapshot_iter, track_indiv_loss=False, offset_iter=0):
        ...
        if self.isLogging:
            layers = list(self.solver.net.params.keys()); #params is an ordered dict, so keys are ordered
            layer_param_shapes = {};
